        ...
        <div class="card">
            <div class="card-body">
                <div class="form-group row">
                    <input type="hidden" id="name_values" name="name_values" value="titleHeaderField">
                    <label for="inputPassword" class="col-sm-2 col-form-label">Title Header</label>
                ...
                <div class="form-group row">
                    <input type="hidden" id="name_values1" name="name_values" value="titleSubHeaderField">
                    <label for="inputPassword" class="col-sm-2 col-form-label">Content Header</label>
                    <div class="col-sm-8 mb-2">
                        <select name="type_fonts" id="type_fonts1" class="form-control select2bs4" style="width: 100%;">
                            <option value="">Change Font</option>
                            @foreach ($fonts as $item)
                            <option value="{{ $item->font }}"
                                {{ old('type_fonts', $fontSubTitle->type_fonts) === $item->font ? 'selected' : NULL }}>
                                {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="card-footer">
                <a href="{{ route('headerFieldFasilitas.index') }}" class="btn btn-info float-right">Back</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $('.btn-title').click(function () {
        ...
        $.ajax({
            url: '{{ route('changeFont.storeTitleHeaderField') }}',
            type: 'POST',
            data: {
